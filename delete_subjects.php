...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkSql = "SELECT * FROM tbl_hocphan WHERE mhp = ?";
    $checkStmt = $dbh->prepare($checkSql);
    $checkStmt->execute([$ma_hp]);
    $subject = $checkStmt->fetch();

    if ($subject) {
        // Nếu sinh viên tồn tại, thực hiện xoá
        try {
            $sql = "DELETE FROM tbl_hocphan WHERE mhp = ?";
            $stmt = $dbh->prepare($sql);

            if($stmt->execute([$ma_hp])) {
                $msg = "Xoá thành công";
            } else {
                $msg = "Xoá thất bại";
            }
        ...
        <div class="panel-body p-20">
            <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                <div class="form-group has-success">
                    ...
                    <div class="">
                        <input type="text" name="ma_hp" class="form-control" required="required" id="success">
                    </div>
                </div>
